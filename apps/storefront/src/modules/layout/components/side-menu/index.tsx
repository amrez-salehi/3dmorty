...
const SideMenu = ({ regions, locales, categories, currentLocale }: SideMenuProps) => {
  ...
  const copy = isPersian
    ? {
        ...
        close: "بستن منو",
        eyebrow: "3DMorty / PRINTED COLLECTIBLES",
        title: "کلکسیون خودت را بساز",
        shop: "خرید همه محصولات",
        decor: "فیگورها و کلکسیونی",
        edit: "محصولات تازه",
        account: "حساب کاربری",
        cart: "سبد خرید",
        language: "زبان",
        country: "کشور",
        footer: "© 3DMorty. همه حقوق محفوظ است.",
      }
    : {
        menu: "Categories",
        close: "Close menu",
        ...
        decor: "Figures & collectibles",
        edit: "New arrivals",
        account: "Account",
        cart: "Cart",
        language: "Language",
        country: "Country",
        footer: "© 3DMorty. All rights reserved.",
      }

  const menuItems = [
    { name: copy.shop, href: "/store", featured: true },
    { name: copy.decor, href: "/categories/figures-collectibles" },
    ...(categories || []).slice(0, 6).filter((category) => category.handle !== "figures-collectibles").map((category) => ({ name: isPersian ? getPersianCategoryName(category.handle, category.name) : category.name, href: `/categories/${category.handle}` })),
